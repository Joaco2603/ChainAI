...
describe('ModelRegistry', function () {
  ...
  const DOCKER_URL_1 = 'docker.io/ai-models/gpt-model:v1';
  const DOCKER_URL_2 = 'docker.io/ai-models/llama-model:v2';
  const DOCKER_URL_3 = 'docker.io/ai-models/claude-model:v1';

  beforeEach(async function () {
    // Get signers
    [owner, developer1, developer2, user1, user2, user3] =
      await ethers.getSigners();

    // Deploy contract
    const ModelRegistryFactory = await ethers.getContractFactory(
      'ModelRegistry'
    );
    modelRegistry = await ModelRegistryFactory.deploy();
  ...
  describe('Model Registration', function () {
    it('Should register a new model', async function () {
      ...
      const event = receipt?.logs.find(
        (log: any) => log.fragment?.name === 'ModelRegistered'
      );
    ...
    it('Should emit ModelRegistered event with correct parameters', async function () {
      ...
      await expect(tx)
        ...
        .withArgs(0, developer1.address, DOCKER_URL_1, block!.timestamp);
    });
    ...
    it('Should revert when registering with empty URL', async function () {
      await expect(
        modelRegistry.connect(developer1).registerModel('')
      ).to.be.revertedWithCustomError(modelRegistry, 'EmptyDockerUrl');
    });
  ...
  describe('Model Rating', function () {
    ...
    it('Should prevent rating below minimum', async function () {
      await expect(
        modelRegistry.connect(user1).rateModel(0, 0)
      ).to.be.revertedWithCustomError(modelRegistry, 'InvalidRating');
    });

    it('Should prevent rating above maximum', async function () {
      await expect(
        modelRegistry.connect(user1).rateModel(0, 6)
      ).to.be.revertedWithCustomError(modelRegistry, 'InvalidRating');
    });
    ...
    it('Should prevent double rating by same user', async function () {
      ...
      await expect(
        modelRegistry.connect(user1).rateModel(0, 4)
      ).to.be.revertedWithCustomError(modelRegistry, 'AlreadyRated');
    });

    it('Should prevent rating non-existent model', async function () {
      await expect(
        modelRegistry.connect(user1).rateModel(999, 5)
    ...
    it('Should track user ratings correctly', async function () {
      ...
      expect(await modelRegistry.hasUserRated(0, user2.address)).to.equal(
        false
      );
    });
    ...
    it('Should prevent rating inactive models', async function () {
      ...
      await expect(
        modelRegistry.connect(user1).rateModel(0, 5)
  ...
  describe('Top Models', function () {
    ...
    it('Should return top models sorted by rating', async function () {
      const [topModelIds, ratings] = await modelRegistry.getTopModels(3);
  ...
  describe('Model Selection', function () {
    ...
    it('Should select a model and increment counter', async function () {
      ...
      const selectedModelId = receipt?.logs.find(
        (log: any) => log.fragment?.name === 'ModelSelected'
      );

      expect(selectedModelId).to.not.be.undefined;

      const modelInfo = await modelRegistry.getModelInfo(0);
      const modelInfo1 = await modelRegistry.getModelInfo(1);

      expect(modelInfo.timesSelected + modelInfo1.timesSelected).to.equal(1);
    });

    it('Should emit ModelSelected event', async function () {
      await expect(modelRegistry.connect(user1).selectModel()).to.emit(
        modelRegistry,
        'ModelSelected'
      );
    });
    ...
    it('Should revert when no active models', async function () {
      ...
      await expect(
        modelRegistry.connect(user1).selectModel()
      ).to.be.revertedWithCustomError(modelRegistry, 'NoActiveModels');
    });
  });
  ...
  describe('Model Activation/Deactivation', function () {
    ...
    it('Should emit ModelDeactivated event', async function () {
      const tx = await modelRegistry.connect(developer1).deactivateModel(0);
      const receipt = await tx.wait();
      const block = await ethers.provider.getBlock(receipt!.blockNumber);
    ...
    it('Should prevent non-owner from deactivating model', async function () {
      await expect(
        modelRegistry.connect(user1).deactivateModel(0)
      ).to.be.revertedWithCustomError(modelRegistry, 'OnlyModelOwner');
    });
    ...
    it('Should emit ModelReactivated event', async function () {
      ...
      const tx = await modelRegistry.connect(developer1).reactivateModel(0);
      const receipt = await tx.wait();
      const block = await ethers.provider.getBlock(receipt!.blockNumber);
  ...
  describe('Query Functions', function () {
    ...
    it('Should get models by owner', async function () {
      const dev1Models = await modelRegistry.getModelsByOwner(
        developer1.address
      );
      const dev2Models = await modelRegistry.getModelsByOwner(
        developer2.address
      );
    ...
    it('Should check if user has rated', async function () {
      ...
      expect(await modelRegistry.hasUserRated(1, user1.address)).to.equal(
        false
      );
    });
  });
  ...
  describe('Edge Cases', function () {
    ...
    it('Should handle single model scenario', async function () {
      ...
      const selectedModelId = await modelRegistry.connect(user1).selectModel();
      // With only one model, it should always be selected
      expect(selectedModelId).to.not.be.undefined;
    });
